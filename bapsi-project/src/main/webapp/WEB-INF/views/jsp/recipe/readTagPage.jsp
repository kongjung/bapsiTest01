...
<body>
	...
	<section class="features12 cid-r5sLXhqF4t" id="features12-2p">
		<input type="hidden" name="no" value="${recipe.no}" /> <input
			type="hidden" name="page" value="${tCri.page}" /> <input
			type="hidden" name="numPerPage" value="${tCri.numPerPage}" />
 	  <input type="hidden" name="findType" value="${tCri.findType}" />
	  <input type="hidden" name="keyword" value="${tCri.keyword}" />
		<div class="container">
					...
					<div class="card pl-3 pr-3 pb-5">
						...
						<div class="card-box">
							<c:forEach items="${ tagList }" var="tag" varStatus="loop">
								<a
									href="${pageContext.request.contextPath}/recipe/recipeTagList?page=${tCri.page}&numPerPage=${tCri.numPerPage}&TagName=${tag.tagName}&findType=${tCri.findType}&keyword=${tCri.keyword}">
            ...
            success : function(result) {
               ...
               if (result == 'Success') {
				...
				<button type="button"
        		 ...
        		 &nbsp;&nbsp;
				<button type="button"
					onclick="location.href='${pageContext.request.contextPath}/recipe/delPage?no=${recipe.no}&page=${tCri.page}&numPerPage=${tCri.numPerPage}&findType=${tCri.findType }&keyword=${tCri.keyword }'">삭제</button>
				&nbsp;&nbsp;
				<button type="button"
					onclick="location.href='${pageContext.request.contextPath}/recipe/recipeTagList?page=${tCri.page}&numPerPage=${tCri.numPerPage}&TagName=${tag}&findType=${tCri.findType }&keyword=${tCri.keyword }'">목록</button>
				&nbsp;&nbsp;<br> <br> <br> <br>
				<hr>
