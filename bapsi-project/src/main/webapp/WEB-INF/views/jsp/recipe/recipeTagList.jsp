...
<body>
	...
	<section class="features3 cid-r4QcxVNCog" id="features3-17">
	 <h1 align="center" style="color: blue;"># ${tagName }</h1> <br><br>
	 <div class="media-container-row">
         <div class="btn-group" role="group">
            ...
            <input type="text" size="50" name="keyword" id="findword" value="${tCri.keyword}" />
            <input type="hidden" size="50" name="tagName" id="tagName" value="${tCri.tagName}" />
		...
		<div class="container">
			...
			<div class="media-container-row">			
				<c:forEach begin="0" end="2" items="${recipeList}" var="recipe" varStatus="loop">
			...
			<div class="media-container-row">
				<c:forEach begin="3" end="5" items="${recipeList}" var="recipe" varStatus="loop">
					<div class="card p-3 col-12 col-md-6 col-lg-4">
						<div class="card-wrapper">
							<div class="card-img">
								<a
...
<%-- 								<a
			...
			<div class="btn-group" role="group" aria-label="Basic example"
				...
				<c:if test="${TpagingMaker.prev}">
					<button type="button" class="btn btn-outline-danger btn-sm"
						onclick="location.href='${pageContext.request.contextPath}/recipe/recipeTagList${TpagingMaker.makeFind(TpagingMaker.startPage-1)}'">◀</button>
				</c:if>

				<c:forEach begin="${TpagingMaker.startPage}"
					end="${TpagingMaker.endPage}" var="pNum">
					<a
						href="${pageContext.request.contextPath}/recipe/recipeTagList${TpagingMaker.makeFind(pNum)}">
						<button type="button"
							class="<c:out value="${TpagingMaker.cri.page == pNum?'btn btn-outline-danger btn-sm  active':' btn  btn-default btn-sm'}"/>">
							${pNum}</button>
					</a>
				</c:forEach>
				<c:if test="${TpagingMaker.next&&pagingMaker.endPage>0}">
					<button type="button" class="btn btn-outline-danger btn-sm"
						onclick="location.href='${pageContext.request.contextPath}/recipe/recipeTagList${TpagingMaker.makeFind(TpagingMaker.endPage+1)}'">▶</button>
				</c:if>
			</div>
			</div>
		</div>
 ...
 <script type="text/javascript">
	      	...
	      	$(document).ready(function(){
	      		$('#findBtn').on("click",function(e){
	      			self.location="recipeTagList"+"${TpagingMaker.makeURI(1)}"
	      				+"&findType="
